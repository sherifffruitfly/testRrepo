#this looks like the central implementation
...
cumulative_mean <- function(prev_step=c(1, 0), new_data=0)
{
  ...
  new_mean <- ((prev_step[1] - 1) * prev_step[2] /prev_step[1] ) + ((1/prev_step[1]) * new_data)
...
mm_control <- function(vector_data=0)
{
  #temp hardcoding the argument
  data = c(3, 7, 4, 2)
  
  r1 <- cumulative_mean(c(1, 0), data[1])
